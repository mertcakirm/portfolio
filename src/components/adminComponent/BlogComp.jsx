                ...
                <div className="col-12 add-blog-con row justify-content-center" style={{display: "none"}}>
                    <div style={{height: 'fit-content'}} className="row col-4 row-gap-3 justify-content-center">
                        ...
                        <input type="text" className="col-12 login-inp" placeholder="Blog Title" value={blogs.BlogName}
                               onChange={(e) => setBlogs({...blogs, BlogName: e.target.value})}/>
                        <input type="text" className="col-12 login-inp" placeholder="Blog Başlığı"
                               value={blogs.BLOG_Name_tr}
                               onChange={(e) => setBlogs({...blogs, BLOG_Name_tr: e.target.value})}/>
                        <input type="text" className="col-12 login-inp" placeholder="Blog Content"
                               value={blogs.Blog_description}
                               onChange={(e) => setBlogs({...blogs, Blog_description: e.target.value})}/>
                        <input type="text" className="col-12 login-inp" placeholder="Blog Açıklaması"
                               value={blogs.BLOG_desc_tr}
                               onChange={(e) => setBlogs({...blogs, BLOG_desc_tr: e.target.value})}/>
                        <div className={`file-upload ${dragActive ? "drag-active" : ""}`} onDragOver={handleDragOver}
                    ...
                    <div className="col-8" style={{height: '100vh', overflowY: 'auto'}}>
                        ...
                        {contents.map((content) => (
                            <div key={content.id} className="content-box">
                                ...
                                <input type="text" className="login-inp w-100" placeholder="Başlık"
                                       value={content.title_tr}
                                       onChange={(e) => updateContent(content.id, "title_tr", e.target.value)}/>
                                <textarea className="login-inp pt-2 w-100 mt-2" placeholder="Content"
                                          value={content.content_en}
                                          onChange={(e) => updateContent(content.id, "content_en", e.target.value)}/>
                                <textarea className="login-inp pt-2 w-100 mt-2" placeholder="Açıklama"
                                          value={content.content_tr}
                                          onChange={(e) => updateContent(content.id, "content_tr", e.target.value)}/>
                                ...
                                <button className="btn btn-danger mt-2" onClick={() => removeContent(content.id)}>Sil
                                </button>
                            </div>
                        ))}
                    </div>
